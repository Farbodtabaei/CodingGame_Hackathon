"""\
STRATEGIC BOT V21 (Standalone)
==============================

This file is intentionally self-contained for easy CodinGame submission.

Base: copies the latest V20 build/pathing logic (unplaceable-aware A*, stable two-ended build).
Add-on: conservative predictive disruption override that tries to ink 1-hit (or emergency 2-hit)
regions on high-confidence opponent routes, but only when it is a strict upgrade over the
baseline disruption pick.
"""
...
class StrategicBotV21:
    ...
    PRED_DISABLE_OVERRIDE_WHEN_AHEAD = 120
    PRED_EXTRA_MARGIN_IF_BASE_1HIT = 2.75
    PRED_ALLOW_OVERRIDE_IF_BASE_1HIT_BEHIND = 60
