"""\
STRATEGIC BOT V27 (V25-derived, standalone)
==============================

This file is intentionally self-contained for easy CodinGame submission.

Base: V25 standalone (V20 build/pathing + conservative predictive disruption).
Add-on: optional (env-var gated) dataset logging + forced-disrupt hooks for NN/oracle pipelines.
Default gameplay is identical to V25 unless dataset/force env vars are set.
"""
...
class StrategicBotV27:
    ...
    TEMPO_ENABLED = False
    TEMPO_TURN_MIN = 18
    TEMPO_SCORE_DELTA_MAX = 30
    TEMPO_EV_WINDOW = 30.0
    TEMPO_TOP_K = 12
    TEMPO_MAX_TURNS_TO_COMPLETE = 2
    TEMPO_BONUS = 60.0
    TEMPO_W_FAST = 0.0
    ...
    def build_path_two_ended_stable(self, from_town: Town, to_town: Town, budget_override: Optional[int] = None) -> List[str]:
        actions: List[str] = []
    ...
    def get_action(self) -> str:
        ...
        if self.active_build and self.active_build_path:
            ...
            build_actions = self.build_path_two_ended_stable(from_town, to_town, self._remaining_budget_after_actions(actions))
            actions.extend(build_actions)

            if not build_actions and best_new:
                from_id, to_id, path, _ = best_new
                self.active_build = (from_id, to_id)
                self.active_build_path = path
                from_town = self.town_by_id[from_id]
                to_town = self.town_by_id[to_id]
                actions.extend(self.build_path_two_ended_stable(from_town, to_town, self._remaining_budget_after_actions(actions)))
...
def main():
    ...
    while True:
        try:
            ...
            try:
                print(f"V27 ERROR: {type(e).__name__}: {e}", file=sys.stderr)
            except Exception:
                pass
            try:
                with open("v27_crash.log", "a", encoding="utf-8") as f:
                    f.write("\n" + "=" * 80 + "\n")
