"""
STRATEGIC BOT V20 - Adaptive Two-Ended Build + Unplaceable-Aware Pathing
========================================================================

V20 builds on V19 and focuses on converting paint into progress more reliably.

Key upgrades vs V19:
1) Unplaceable-aware planning:
   - If a cell would cost > POINTS_PER_TURN to place (e.g., cost 4 when budget is 3),
     it is effectively impossible to ever build on that cell.
   - V20 treats those empty cells as blocked in A* so it doesn't plan through
     impossible terrain.

2) Adaptive build execution:
   - Within a single turn (up to 3 paint), V20 re-plans after each placed track.
   - This lets it naturally route around expensive/blocked next-steps and keep
     spending paint efficiently, while still building a connected route.

Keeps:
- Deterministic N/E/S/W tie-break A*
- V17/V18 safe disruption logic
- Low compute (max 3 replans per turn)
...
class StrategicBotV20:
    ...
    def _get_cell_cost(self, x: int, y: int) -> float:
        ...
        # Empty cell: if it's impossible to ever place (cost > budget), treat as blocked.
    ...
    def build_path_two_ended_stable(self, from_town: Town, to_town: Town) -> List[str]:
        """Build from both ends along a stable planned path; replan only if stuck."""
        actions: List[str] = []
        ...
        while budget > 0:
            ...
            if not feasible:
                # Replan once when stuck; this avoids oscillating every placement.
    ...
    def get_disruption_target(self) -> Optional[int]:
        ...
        for region_id in set(list(self.active_weight_by_region_opp.keys()) + list(self.active_weight_by_region_my.keys()) + list(self.active_weight_by_region_neutral.keys())):
        ...
        if not candidates:
            for region_id in self.opp_track_regions:
                ...
                opp_tracks = sum(
                    1 for (x, y), owner in self.cell_tracks.items()
    ...
    def get_action(self) -> str:
        ...
        if self.turn >= 1:
            target = self.get_disruption_target()
